...
class FloatingEye:
    def __init__(self, scale, center_x, center_y):
        ...
        self.pupil = arcade.Sprite("assets/gui/info/pupil.png", scale * 0.7)
...
class InfoWindow(arcade.Sprite):
    def __init__(self, filename, window_width, window_height, scale=2):
        ...
        self.arcade_logo = arcade.Sprite("assets/gui/info/arcade_logo.png", scale=0.5)
    ...
    def update_floating_eyes(self, mouse_x, mouse_y):
        for eye in self.floating_eyes:
            ...
            if eye.y_position > self.height:
                eye.y_position = 0
                eye.eye_globe.center_x = random.randint(
                    int(self.left + 30), int(self.right - 30)
                )
    ...
    def update_eye_center_position(self, mouse_x, mouse_y):
        ...
        angle = atan2(
            mouse_y - self.eye_globe.center_y, mouse_x - self.eye_globe.center_x
        )
        distance_to_cursor = (
            (self.eye_globe.center_x - mouse_x) ** 2
            + (self.eye_globe.center_y - mouse_y) ** 2
        ) ** 0.5
        ...
        self.pupil_scale_target = 1.75 - (
            min(distance_to_cursor, max_distance) / max_distance
        )

        self.eye_center.center_x = (
            self.eye_globe.center_x + cos(angle) * distance_to_pupil
        )
        self.eye_center.center_y = (
            self.eye_globe.center_y + sin(angle) * distance_to_pupil
        )
        ...
        self.pupil.scale = (
            self.pupil.scale + (self.pupil_scale_target - self.pupil.scale) * progress
        )
    ...
    def update(self, window_width, window_height, mouse_x, mouse_y):
        ...
        if delta_x != 0 or delta_y != 0:
            ...
            for eye in self.floating_eyes:
                ...
                eye.eye_globe.center_y = random.randint(
                    int(self.bottom + 30), int(self.top - 30)
                )
